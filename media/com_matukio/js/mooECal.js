...
var Calendar = new Class({
	...
	initialize: function(options){
        ...
        this.timeTwelveHourFormat = this.options.timeTwelveHourFormat;
        this.dayText = this.options.dayText;
        this.weekText = this.options.weekText;
        this.monthText = this.options.monthText;
        this.setCalParams();
			...
			var liViewPicker = new Element('li',{
				...
				var ulViewPicker = new Element('ul',{
					...
					var liMonthPicker = new Element('li', {
						...
						}).set('text',this.monthText).inject(liMonthPicker);
					var liWeekPicker = new Element('li', {
						...
						}).set('text',this.weekText).inject(liWeekPicker);
					var liDayPicker = new Element('li', {
						...
						}).set('text',this.dayText).inject(liDayPicker);
	...
	getCalEvents: function(){
		if ((!this.gotEvents || this.viewStartDate < this.eventRangeStart || this.viewEndDate > this.eventRangeEnd) && this.options.feedPlugin != '') {
			...
			$('loading').fade('in');
			new mecPHPPlugin().getEvents(this,this.eventRangeStart,this.eventRangeEnd)
		}
		else
			this.loadCalEvents();
	},
	loadCalEvents: function(){
		...
		for(var i = 0; i < this.options.cEvents.length; i++){
			...
			time = (this.options.cEvents[i].start.toString().length > 16) ? eStart.thTime(this.timeFormat, this.timeTwelveHourFormat) : ''; // check to see if feed event date included a time
