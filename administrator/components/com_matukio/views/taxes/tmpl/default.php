...
echo CompojoomHtmlCtemplate::getHead(MatukioHelperUtilsBasic::getMenu(), 'taxes', 'COM_MATUKIO_TAXES', 'COM_MATUKIO_SLOGAN_TAXES');
...
?>
	...
	<form action="<?php echo JRoute::_("index.php?option=com_matukio&view=taxes"); ?>" method="post" name="adminForm" id="adminForm">
		...
		<div id="filter-bar" class="btn-toolbar">
			...
			<div class="filter-select fltrt pull-right">
				<select name="filter_status" class="inputbox chzn-single" onchange="this.form.submit()">
					<option value=""><?php echo JText::_('COM_MATUKIO_ALL_TAXES'); ?></option>
		...
		<table class="adminlist table table-hover">
				...
				foreach ($this->items as $i => $l)
				{
					...
					$published = JHTML::_('grid.published', $l, $i, 'tick.png', 'publish_x.png', 'taxes.');

					$link = JRoute::_('index.php?option=com_matukio&controller=taxes&task=editTax&id=' . $l->id);
					?>
