	...
	<form name="adminForm" id="adminForm" method="post"
		...
		<table class="adminlist table table-hover" style="width: 100%;">
			...
			<?php foreach ($this->items as $i => $item) :
				if (empty($item->name))
				{
					$item->name = JFactory::getUser($item->userId)->name;
				}

				if (empty($item->email))
				{
					$item->email = JFactory::getUser($item->userId)->email;
				}
				...
				<tr class="row<?php echo $i % 2; ?>">
					...
					<td>
						<div align="center"><?php echo '<a href="index.php?option=com_matukio&view=editorganizer&id=' . $item->id . '">'
								. $item->userId . '</a>'; ?></div>
					</td>
					<td><?php echo $item->id; ?></td>
					<td><?php
						echo '<a href="index.php?option=com_matukio&view=editorganizer&id=' . $item->id . '">' . $item->name . '</a>';
						?>
					</td>
